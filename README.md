...
>
> **Windows 安全提醒：** SmartScreen 信譽提示與 Defender Antivirus 隔離是兩回事。遇到防毒警示請先查閱 [Windows 下載與安全驗證說明](./WINDOWS_SECURITY.md)，使用 `SHA256SUMS.txt` 比對 GitHub 原生建置產物；若備用包也被偵測，請停止並回報，不要直接停用保護。
