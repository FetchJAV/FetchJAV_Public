...
<p align="center">
  <strong><a href="https://github.com/Alos21750/FetchJAV/releases/latest/download/JableTV_Modern.exe">下載 Modern</a></strong>
  ·
  <strong><a href="https://github.com/Alos21750/FetchJAV/releases/latest/download/Jable_smalltool.exe">下載 SmallTool</a></strong>
  ·
  <a href="https://github.com/Alos21750/FetchJAV/releases/latest">SmallTool portable ZIP（v2.5.38 起）</a>
  ·
  <a href="https://github.com/Alos21750/FetchJAV/releases/latest">查看最新版本</a>
</p>
...
<p align="center">
  <img src="./img/readme_modern.png" width="100%" alt="FetchJAV Modern v2.5.35 English dark interface with JableTV, MissAV and SupJav browse tabs" />
</p>
...
1. 下載 [JableTV_Modern.exe](https://github.com/Alos21750/FetchJAV/releases/latest/download/JableTV_Modern.exe) 或 [Jable_smalltool.exe](https://github.com/Alos21750/FetchJAV/releases/latest/download/Jable_smalltool.exe)。
...
下載記錄優先存於執行檔旁的 `.Jable_smalltool`；若該位置不可寫，會改用 `%APPDATA%\FetchJAV\smalltool`。
...
- 日文辨識全程在本機執行。預設的 **自動（建議）** 使用固定 revision 的 [ReazonSpeech K2 v2](https://huggingface.co/reazon-research/reazonspeech-k2-v2) 與 [sherpa-onnx](https://github.com/k2-fsa/sherpa-onnx)，針對一般 CPU 最佳化，不需獨立顯示卡；經 SHA-256 驗證的辨識包約 **178 MB**，只在第一次實際要求字幕時下載。
- 原有的 [whisper.cpp](https://github.com/ggml-org/whisper.cpp) 模式仍可明確選用：**精準 large-v3-turbo q5（約 574 MB）**、**平衡 small q5（約 190 MB）**、**快速 base q5（約 60 MB）**。自動模式只會在執行失敗、輸出損壞或時間軸結構無效時另行下載並改用平衡模式；不會因為猜測文字內容好壞而擅自切換。
- [官方 Silero VAD](https://huggingface.co/ggml-org/whisper-vad/tree/main) 只作為語音閘門；辨識會使用保留原始靜音的、具上下文且互不重疊的視窗，再把結果映射回影片的絕對時間。ReazonSpeech 使用 CPU 高效率的 RNN-T 解碼；三個 Whisper 模式則使用 beam search、best-of 與溫度 fallback。
...
開 [GitHub Issue](https://github.com/Alos21750/FetchJAV/issues/new) 時，請提供：
...
<p align="center">
  <img src="./img/star-history.svg" width="100%" alt="Verified GitHub star history for FetchJAV" />
</p>
...
版本變更與已修復問題請看 [Releases](https://github.com/Alos21750/FetchJAV/releases)。
