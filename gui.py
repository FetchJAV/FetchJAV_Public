    ...
    def __init__(self, master, **kw):
        ...
        def _auto_sb_set(first, last):
            try:
                ...
                pass
            sb.set(first, last)
        self.configure(yscrollcommand=_auto_sb_set)
        sb.pack(side=tk.RIGHT, fill='y')
...
class MainWindow(tk.Tk):
    ...
    def _build_dl_tab(self, parent):
        ...
        csb = ttk.Scrollbar(console_frame, orient=tk.VERTICAL,
                            ...
                            style='Vertical.TScrollbar')
        def _auto_csb_set(first, last):
            try:
                f, l = float(first), float(last)
                if f <= 0.001 and l >= 0.999:
                    if csb.winfo_ismapped():
                        csb.pack_forget()
                else:
                    if not csb.winfo_ismapped():
                        csb.pack(side=tk.RIGHT, fill='y')
            except Exception:
                pass
            csb.set(first, last)
        self._console.configure(yscrollcommand=_auto_csb_set)
        csb.pack(side=tk.RIGHT, fill='y')
    ...
    def _build_settings_tab(self, parent):
        ...
        tk.Label(grp2, text='FetchJAV GUI',
